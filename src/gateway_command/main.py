...
if __name__ == '__main__':
    ...
    tests = [name for name in os.listdir(os.path.dirname(os.path.realpath(__file__)) + "\\gateway_command")
    ...
    for test in tests_to_run:
        exec(
            open(
                os.path.dirname(os.path.realpath(__file__)) + "\\gateway_command\\" + tests[test - 1]
            ).read(),
            globals(),
            locals()
        )
